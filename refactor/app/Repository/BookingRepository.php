...
class BookingRepository extends BaseRepository
{
    ...
    public function store(User $user, array $data)
    {
        ...
        if ($user->is('customer')) {
            ...
            if ($data['immediate'] == 'no') {
                if (empty($data['due_date']) || empty($data['due_time']) || !isset($data['customer_phone_type']) || empty($data['duration'])) {
                    return [
                        'status' => 'fail',
                        'message' => 'Du måste fylla in alla fält',
                        'field_name' => $this->getEmptyField($data, ['due_date', 'due_time', 'customer_phone_type', 'duration'])
                    ];
                }
            } else {
    ...
    public function storeJobEmail(array $data)
    {
        ...
        if (!empty($address)) {
            ...
            $job->address = $user->userMeta->address;
        }

        if (!empty($instructions)) {
            $job->instructions = $instructions;
        } else {
            $job->instructions = $user->userMeta->instructions;
        }

        if (!empty($town)) {
            $job->town = $town;
        } else {
            $job->town = $user->userMeta->city;
        }
    ...
    public function jobEnd(array $post_data)
    {
        ...
        $this->mailer->send($translatorEmail, $translatorName, $subject, 'emails.session-ended', $data);
    ...
    public function sendNotificationTranslator($job, $data = [], $exclude_user_id)
    {
        ...
        foreach ($suitableTranslators as $translator) {
            ...
            $potentialJobs = $this->getPotentialJobIdsWithUserId($translator->id); // get all potential jobs of this user
            foreach ($potentialJobs as $potentialJob) {
                if ($job->id == $potentialJob->id) { // one potential job is the same with the current job
                    ...
                    if ($jobForTranslator == 'SpecificJob') {
                        ...
                        if ($jobChecker != 'userCanNotAcceptJob') {
                            if ($this->isNeedToDelayPush($translator->id)) {
                                $delayedTranslatorArray[] = $translator;
                            } else {
                                $translatorArray[] = $translator;
                            }
                        }
                    }
                }
        ...
        $this->sendPushNotificationToSpecificUsers($translatorArray, $job->id, $data, $msgText, false); // send new booking push to suitable translators (not delayed)
        $this->sendPushNotificationToSpecificUsers($delayedTranslatorArray, $job->id, $data, $msgText, true); // send new booking push to suitable translators (need to be delayed)
    }
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        // determine the message template based on job type
        $messageTemplate = '';
